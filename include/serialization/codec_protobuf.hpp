// codec_protobuf.hpp - Protobuf 风格编码器 (自研 wire format, 不依赖 libprotobuf)
// magic: "LCPB"
...
public:
    ...
    void write_i32(int32_t v) noexcept override {
        if (stack_.size() == 0) return;
        auto& f = stack_.back();
        // int32 用 varint (负数会被扩展为 10 字节, 可优化为 zigzag)
        pb_detail::write_tag(f.buf, ++f.field_idx, pb_detail::wire_type::varint);
        pb_detail::write_varint(f.buf, static_cast<uint64_t>(static_cast<int64_t>(v)));
    }
    ...
    void write_i64(int64_t v) noexcept override {
        ...
        auto& f = stack_.back();
        pb_detail::write_tag(f.buf, ++f.field_idx, pb_detail::wire_type::varint);
        pb_detail::write_varint(f.buf, static_cast<uint64_t>(v));
    }
...
public:
    ...
    bool enter_object() noexcept override {
        ...
        if (err_ || cur.p + len > cur.end) { err_ = true; return false; }
        frame sub;
        sub.p = cur.p;
        sub.end = cur.p + len;
        cur.p += len;
    ...
    [[nodiscard]] int32_t read_i32() noexcept override {
        return static_cast<int32_t>(static_cast<int64_t>(
            pb_detail::read_varint(cur_p(), cur_end(), err_)));
    }
    ...
    [[nodiscard]] int64_t read_i64() noexcept override {
        return static_cast<int64_t>(pb_detail::read_varint(cur_p(), cur_end(), err_));
    }
    ...
    [[nodiscard]] float read_f32() noexcept override {
        if (cur_p() + 4 > cur_end()) { err_ = true; return 0.0f; }
    ...
    [[nodiscard]] double read_f64() noexcept override {
        if (cur_p() + 8 > cur_end()) { err_ = true; return 0.0; }
    ...
    [[nodiscard]] std::string_view read_string_view() noexcept override {
        ...
        if (err_ || cur_p() + len > cur_end()) { err_ = true; return {}; }
