...
public:
    ...
    [[nodiscard]] std::string_view read_bytes_view(size_t len) noexcept override {
        // JSON 路径: bytes 存为 base64 字符串, 解码后返回临时缓冲区
        // 注意: 此处返回的视图指向内部临时缓冲区, 不可长期持有
        (void)len;  // JSON 长度由流内 base64 内容决定
        std::string_view b64 = r_.read_string();
        std::string decoded = detail::base64_decode(b64);
        // 零拷贝妥协: trivial 组件数据在 JSON 路径需解码
        // 返回的视图生命周期绑定到 reader 内部, 调用方应立即 memcpy
        tmp_ = std::move(decoded);
        return std::string_view(tmp_);
    }
...
private:
    std::string tmp_;  // base64 解码临时缓冲区
};
