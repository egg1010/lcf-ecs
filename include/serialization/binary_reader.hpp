...
public:
    ...
    binary_reader& skip(size_t n) noexcept {
        if (p_ + n > end_) { err_ = true; return *this; }
        p_ += n;
        return *this;
    }

    [[nodiscard]] uint32_t read_u32() noexcept {
        if (p_ + 4 > end_) { err_ = true; return 0; }
    ...
    [[nodiscard]] uint64_t read_u64() noexcept {
        if (p_ + 8 > end_) { err_ = true; return 0; }
    ...
    [[nodiscard]] float read_f32() noexcept {
        if (p_ + 4 > end_) { err_ = true; return 0.0f; }
        float v;
    ...
    [[nodiscard]] double read_f64() noexcept {
        if (p_ + 8 > end_) { err_ = true; return 0.0; }
        double v;
    ...
    [[nodiscard]] bool read_bool() noexcept {
        if (p_ + 1 > end_) { err_ = true; return false; }
        bool v = (*p_ != 0);
        p_ += 1;
        return v;
    }
    ...
    [[nodiscard]] std::string_view read_string_view() noexcept {
        ...
        if (err_ || p_ + len > end_) { err_ = true; return {}; }
    ...
    void read_trivial(T& out) noexcept {
        if (p_ + sizeof(T) > end_) { err_ = true; return; }
