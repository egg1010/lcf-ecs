// 容量/访问/迭代器接口
    ...
    [[nodiscard]] FORCE_INLINE const_iterator begin() const noexcept
    {
        ...
        ensure_cp_count();
        if (cp_count_ > 0) build_cp_cache();
        if (cp_cache_) return const_iterator(cp_cache_);
        return const_iterator(static_cast<const char32_t*>(nullptr));
    }
    [[nodiscard]] FORCE_INLINE const_iterator end() const noexcept
    {
        if (cp_cache_) [[likely]] return const_iterator(cp_cache_ + cp_count_);
        ensure_cp_count();
        if (cp_count_ > 0) build_cp_cache();
        if (cp_cache_) return const_iterator(cp_cache_ + cp_count_);
        return const_iterator(static_cast<const char32_t*>(nullptr));
    }
    [[nodiscard]] const_iterator cbegin() const noexcept { return begin(); }
    [[nodiscard]] const_iterator cend() const noexcept { return end(); }

    // 重建 cp_cache_ 并返回指向码点 idx 的迭代器 (insert/erase 后用)
    [[nodiscard]] const_iterator make_iterator(size_t idx) const noexcept
    {
        if (!cp_cache_)
        {
            ensure_cp_count();
        ...
        if (!cp_cache_) return const_iterator(static_cast<const char32_t*>(nullptr));
        if (idx >= cp_count_) return const_iterator(cp_cache_ + cp_count_);
        return const_iterator(cp_cache_ + idx);
    }
    ...
    {
        ensure_cp_count();
        return const_reverse_iterator(data_ + byte_size_, data_, data_ + byte_size_, uniform_byte_len_);
    }
    [[nodiscard]] const_reverse_iterator rend() const noexcept
    {
        ensure_cp_count();
        return const_reverse_iterator(data_, data_, data_ + byte_size_, uniform_byte_len_);
    }
