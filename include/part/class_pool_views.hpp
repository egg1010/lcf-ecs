...
// 跨平台宏 (CPV_PREFETCH_R / CPV_ALWAYS_INLINE / CPV_FLATTEN / CPV_RESTRICT):
...
namespace cpv
{
CPV_ALWAYS_INLINE void compiler_barrier() noexcept
{
...
// A. 子范围视图 (与 dense::subspan / std::span::subspan 命名一致)
template <typename T>
...
CPV_FLATTEN inline void reverse_for_each(class_pool<T>& pool, F&& f) noexcept
{
...
template <typename T, typename F>
CPV_FLATTEN inline void reverse_for_each(const class_pool<T>& pool, F&& f) noexcept
{
...
// C. 步进视图 (POD 视图结构, 与 dense::strided_span 命名一致)
...
struct strided_span
{
    ...
    constexpr strided_span(class_pool<T>* p, size_t s, size_t st, size_t c) noexcept
    ...
    template <typename F>
    CPV_FLATTEN void for_each(F&& f) noexcept
    {
        if (pool_->is_dense()) [[likely]]
        {
            T* CPV_RESTRICT p = pool_->data() + start_;
            ...
            for (size_t i = 0; i < n; ++i)
            {
                ...
                {
                    if (i + 8 < n) [[likely]] { CPV_PREFETCH_R(p + 8 * step); }
                }
                f(*p);
                p += step;
            }
...
template <typename T>
[[nodiscard]] inline strided_span<T> strided_span_view(
    class_pool<T>& pool, size_t start, size_t step, size_t count) noexcept
{
    return strided_span<T>(&pool, start, step, count);
}

template <typename T, typename F>
CPV_FLATTEN inline void strided_for_each(
    class_pool<T>& pool, size_t start, size_t step, F&& f) noexcept
{
    if (pool.is_dense()) [[likely]]
    {
        T* CPV_RESTRICT p = pool.data() + start;
        ...
        for (size_t i = 0; i < cnt; ++i)
        {
            if constexpr (sizeof(T) >= 16)
            {
                if (i + 8 < cnt) [[likely]] { CPV_PREFETCH_R(p + 8 * step); }
            }
            f(*p);
            p += step;
        }
    ...
    const class_pool<T>& pool, size_t start, size_t step, F&& f) noexcept
{
    if (pool.is_dense()) [[likely]]
    {
        const T* CPV_RESTRICT p = pool.data() + start;
        const size_t avail = pool.size() > start ? pool.size() - start : 0;
        const size_t cnt = avail / step;
        for (size_t i = 0; i < cnt; ++i)
        {
...
template <typename T, size_t Step, typename F>
    requires (Step > 0)
CPV_FLATTEN inline void strided_for_each(class_pool<T>& pool, F&& f) noexcept
{
    if constexpr (Step == 1)
    {
...
template <typename T, size_t Step, typename F>
    requires (Step > 0)
CPV_FLATTEN inline void strided_for_each(const class_pool<T>& pool, F&& f) noexcept
{
    if constexpr (Step == 1)
    {
...
CPV_FLATTEN inline void transform_for_each(
...
inline void transform_to(const class_pool<T>& pool, R* dst, size_t count, F&& tr) noexcept
{
    ...
    if (pool.is_dense()) [[likely]]
    {
        const T* CPV_RESTRICT src = pool.data();
        for (size_t i = 0; i < n; ++i)
        {
...
CPV_FLATTEN inline void filter_for_each(class_pool<T>& pool, Pred pred, F&& f) noexcept
{
    for (auto it = pool.begin(); it != pool.end(); ++it)
    {
        if (pred(*it)) [[likely]]
...
CPV_FLATTEN inline void filter_for_each(const class_pool<T>& pool, Pred pred, F&& f) noexcept
{
    for (auto it = pool.cbegin(); it != pool.cend(); ++it)
    {
        if (pred(*it)) [[likely]]
...
[[nodiscard]] inline U reduce_pairwise(const class_pool<T>& pool, F&& f, U init) noexcept
{
    ...
    if (pool.is_dense()) [[likely]]
    {
        const T* CPV_RESTRICT p = pool.data();
...
CPV_FLATTEN inline void for_each_window(class_pool<T>& pool, F&& f) noexcept
{
...
CPV_FLATTEN inline void for_each_window(const class_pool<T>& pool, F&& f) noexcept
{
    if (pool.size() < N) [[unlikely]] { return; }
    if (pool.is_dense()) [[likely]]
    {
...
template <typename T, size_t N, typename F>
    requires (N > 0) && std::invocable<F, std::span<T, N>>
CPV_FLATTEN inline void for_each_chunk(class_pool<T>& pool, F&& f) noexcept
{
    if (pool.is_dense()) [[likely]]
    {
        T* p = pool.data();
...
template <typename T, size_t N, typename F>
    requires (N > 0) && std::invocable<F, std::span<const T, N>>
CPV_FLATTEN inline void for_each_chunk(const class_pool<T>& pool, F&& f) noexcept
{
    if (pool.is_dense()) [[likely]]
    {
        const T* p = pool.data();
...
CPV_FLATTEN inline void for_each_enumerated(class_pool<T>& pool, F&& f) noexcept
{
    size_t idx = 0;
...
CPV_FLATTEN inline void for_each_enumerated(const class_pool<T>& pool, F&& f) noexcept
{
    size_t idx = 0;
...
CPV_FLATTEN inline void for_each_zip(
    ...
    const class_pool<T>& a, const U* b, size_t count, F&& f) noexcept
{
    ...
    if (a.is_dense()) [[likely]]
    {
        const T* CPV_RESTRICT pa = a.data();
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC ivdep
#endif
        for (size_t i = 0; i < n; ++i)
...
CPV_FLATTEN inline void simd_for_each(class_pool<T>& pool, F&& f) noexcept
{
    if (pool.is_dense() && sizeof(T) <= 32) [[likely]]
    {
        T* CPV_RESTRICT p = pool.data();
        const size_t n = pool.size();
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC ivdep
#endif
...
CPV_FLATTEN inline void simd_for_each(const class_pool<T>& pool, F&& f) noexcept
{
    if (pool.is_dense() && sizeof(T) <= 32) [[likely]]
    {
        const T* CPV_RESTRICT p = pool.data();
