...
private:
	...
	static void relocate_sparse(T* dst, const T* src, const uint64_t* src_bits, size_t count) noexcept {
		...
		for (size_t w = 0; w < num_full_words; ++w) {
			...
			if (w + 1 < num_full_words) {
				PREFETCH_R(&src_bits[w + 1]);
			}
			while (word != 0) {
				const size_t offset = std::countr_zero(word);
...
public:
	...
	public:
		...
		void skip_to_next_valid() noexcept {
			...
			for (++word_idx; word_idx < total_words; ++word_idx) {
				word = bits_[word_idx];
				if (word != 0) {
	...
	inline void shrink_to_fit() noexcept {
		...
		if (usage_ < maximum_quantity_ && usage_ > 0) [[likely]] {
			...
			uint64_t* new_bits = obtain_bitmap(usage_);
			const bool was_inline = is_inline_bitmap();
	...
	inline iterator erase(const_iterator first, const_iterator last) noexcept {
		...
		if (real_end < usage_) [[likely]] {
			if constexpr (std::is_trivially_copyable_v<T>) {
				...
				std::destroy_n(data_ptr_ + usage_ - gap, gap);
			}
			else {
...
private:
	void recompute_is_dense() noexcept {
		if (usage_ == 0) { is_dense_ = true; hole_count_ = 0; return; }
	...
	void append_bulk(const T* src, size_t count) noexcept {
		...
		if constexpr (std::is_trivially_copyable_v<T>) {
			std::memcpy(data_ptr_ + usage_, src, count * sizeof(T));
		}
		else {
			for (size_t i = 0; i < count; ++i) {
				new (data_ptr_ + usage_ + i) T(src[i]);
			}
		}
	...
	void append_bulk_move(T* src, size_t count) noexcept {
		...
		if constexpr (std::is_trivially_copyable_v<T>) {
			...
			for (size_t i = 0; i < count; ++i) {
				new (data_ptr_ + usage_ + i) T(std::move(src[i]));
			}
		}
		size_t end = usage_ + count;
		bulk_set_bits(usage_, end);
	...
	void append_indices_from(const EntityLike* entities, size_t count) noexcept {
		...
		for (size_t i = 0; i < count; ++i) {
			data_ptr_[usage_ + i] = static_cast<T>(entities[i].parts_.index_);
		}
		size_t end = usage_ + count;
		bulk_set_bits(usage_, end);
		usage_ = end;
